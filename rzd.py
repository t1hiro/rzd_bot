r"""
Основано на https://github.com/devbis/aiorzd
"""
...
class RzdFetcher:
    ...
    def _do_autocomplete_request(self, text: str) -> dict:
        url = "{}{}".format(
            self.site_url,
            self.suggest_url,
        )
        ...
        with self.session.get(url, params=params, verify=False) as r:
            try:
                response = _loads(r.text)
    ...
    def get_city_autocomplete(self, name: str) -> dict:
        error = None
        for _ in range(10):
            try:
                response = self._do_autocomplete_request(name)
    ...
    def get_city_code(self, name: str) -> str:
        return (self.get_city_autocomplete(name))['c']
    ...
    def send_query(self, rzd_req: RzdRequest):
        # send search form
        logging.debug(
            " (%s.%s) : %s" % (
        ...
        try:
            for _ in range(10):
                with self.session.post(trains_url, data=post_params, verify=False) as r:
                    result = r.text
                    try:
    ...
    def trains(self, src: str, dst: str, departure_range: TimeRange) \
            -> Iterable[Train]:
        return self.trains_by_code(
            self.get_city_code(src),
            self.get_city_code(dst),
            departure_range,
        )

    def trains_by_code(self, src_code: str, dst_code: str,
